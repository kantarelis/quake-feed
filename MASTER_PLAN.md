...
| 6 | SSE alerts (`/alerts/stream`) | 🟡 In progress | In-process pub/sub, per-key persistent filters, filter matcher. Worker→API hop via Postgres LISTEN/NOTIFY on a quake.events INSERT trigger (locked in `PLAN.md` design choice 1). |
