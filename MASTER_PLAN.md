...
| 8 | Observability (Prometheus metrics + Grafana dashboard) | ✅ Done | Worker `:8001` metrics via `worker_init` + `--pool=threads` (shared registry) + `WORKER_METRICS_PORT`; `functions/metrics.py` ingestion metrics (`usgs_poll_seconds`, `usgs_poll_errors_total`, `events_{inserted,updated,revisions}_total`) wired into `poll_once`; `sse_connections_active` gauge in the subscriber registry; provisioned Grafana dashboard `monitoring/grafana/dashboards/ingestion.json` (5 panels) + datasource-drift unit test; promtail→Loki collection (`monitoring/promtail-config.yml`, `job="quake-feed"`); `docs/observability.md` + `make metrics`. Grew from 5 to 6 tasks mid-epic (Loki collector added — see decisions). Commits `8a58865..cf31767` (incl. `62539ba`, adjacent `make list/prune-api-keys` tooling — not observability proper). Plan archive to `docs/history/epic-08-observability.md` pending. |
| 9 | Documentation polish + architecture diagram + demo GIF | 🟡 In progress | README finalization; `docs/architecture.md`. Branch `polish-documentation-and-architecture-diagram`. Fills the 3 missing subsystem docs (architecture, database-migrations, task-scheduler — currently dead links in README + CLAUDE.md), polishes the existing ones, completes the "What this demonstrates" table, adds the demo GIF, and a doc-integrity test. |
...
| Worker metrics topology | Two scrape targets (API `:8000`, worker `:8001`); worker on a `--pool=threads` pool | 2026-05-22 | `prometheus_client` registries are per-process. Default prefork forks task execution into child procs, scattering metrics away from the `worker_init` HTTP server; a threads pool keeps one process so the registry and exporter are shared. Multiprocess mode rejected as overkill for a single-pod, one-poll-per-minute worker. |
| Log collection → Loki | promtail (`docker_sd_configs`) shipping app containers under `job="quake-feed"` | 2026-05-22 | Design choice 8 had assumed the logging→Loki pipeline already existed; it didn't (logger only wrote stdout, no collector in compose). Added a minimal promtail service mid-epic so the dashboard's Loki panel has data. |
